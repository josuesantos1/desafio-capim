...
const highlightSpecialty = computed(() => dentist.value?.specialties[0] ?? 'Clínico Geral')
  ...
  <section>
    ...
    <template v-else>
      <div v-reveal class="mt-6">
        <span class="eyebrow">{{ dentist.years_of_experience }} anos de experiência</span>
        <h1 class="mt-3">{{ dentist.name }}</h1>
        <p class="text-lg text-neutral-500 dark:text-neutral-400">{{ highlightSpecialty }}</p>
      </div>
        ...
        <div class="shell">
          <div class="card">
            ...
            <p class="text-neutral-500 dark:text-neutral-400">
              {{ dentist.bio || 'Nenhuma biografia informada.' }}
            </p>

            <h2>Especialidades</h2>
            <div v-if="dentist.specialties.length" class="flex flex-wrap gap-2">
              <span v-for="s in dentist.specialties" :key="s" class="eyebrow normal-case">{{ s }}</span>
            </div>
