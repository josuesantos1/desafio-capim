...
<template>
  <div>
    ...
    <form @submit.prevent="submit">
      ...
      <button type="submit" :disabled="store.loading">Adicionar dentista</button>
    </form>

    <h2>Dentistas</h2>
    <p v-if="store.listByClinic(clinicId).length === 0">Nenhum dentista cadastrado.</p>
    <ul v-else>
      <li v-for="dentist in store.listByClinic(clinicId)" :key="dentist.id">
        <template v-if="editingId === dentist.id">
          <form @submit.prevent="submitEdit(dentist.id)">
        ...
        <template v-else>
          <strong>{{ dentist.name }}</strong> — {{ dentist.email }} — {{ dentist.phone }}
          <label>
            <input
              type="checkbox"
          ...
          <button type="button" @click="startEdit(dentist)">Editar</button>
          <button type="button" @click="remove(dentist.id)">Excluir</button>
        </template>
      </li>
    </ul>
  </div>
